...
#define AP_XSENDFILE_INVOKED_HEADER "X-SENDFILE-INVOKED"
...
static const char *ap_xsendfile_get_orginal_path(request_rec *rec) {
    ...
    if (strncmp(rv, rec->uri, uri_len) == 0) {
        ...
        /* need to lookup the url again as it changed -- note this will cause mod_rewrite
           to fire again so we're adding an input header that can be part of the RewriteCond
           to skip any rules that shouldn't re-apply
           ( RewriteCond %{HTTP:X-SENDFILE-INVOKED} !^.+$ )
        */
        apr_table_set(rec->headers_in, AP_XSENDFILE_INVOKED_HEADER, "True");
...
static apr_status_t ap_xsendfile_output_filter(ap_filter_t *f, apr_bucket_brigade *in) {
    ...
    ap_log_rerror(APLOG_MARK, APLOG_TRACE2, 0, r, "xsendfile: output_filter for %s", r->the_request);
    ...
    */
    if (ctx == NULL) {
        ...
        ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r, "xsendfile: called a second time, skipping");
    ...
    if (
        ...
        ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r, "xsendfile: not met [%d]", r->status);
        ap_remove_output_filter(f);
        return ap_pass_brigade(f->next, in);
    }
    ...
    */
    ...
    /* nothing there :p */
    if (!file || !*file) {
        ap_log_rerror(APLOG_MARK, APLOG_TRACE6, 0, r, "xsendfile: nothing found");
        ap_remove_output_filter(f);
        return ap_pass_brigade(f->next, in);
    }
    ...
    */
    ...
    ap_log_rerror(APLOG_MARK, APLOG_TRACE6, 0, r, "dropping content headers");
    ...
    ap_log_rerror(APLOG_MARK, APLOG_TRACE6, 0, r, "decoding file path");
    ...
    ap_log_rerror(APLOG_MARK, APLOG_TRACE6, 0, r, "file path verification");
    ...
    ap_log_rerror(APLOG_MARK, APLOG_TRACE2, 0, r, "xsendfile: found %s", translated);
    ...
    */
    ...
    if ((errcode = ap_meets_conditions(r)) != OK) {
        ...
        /* Update for APR 2.4, where files are inserted as a FILE type bucket.
           For larger files, instead of actually reading all bytes into buckets,
           FILE type buckets read 8K at a time, converting the bucket into a HEAP
           data bucket with the read data and a new FILE type bucket with an offset
           for the next read operation. */


        ap_log_rerror(APLOG_MARK, APLOG_TRACE6, 0, r, "inserting new bucket");

        e = apr_brigade_insert_file(in, fd, 0, (apr_size_t)finfo.size, r->pool);
    ...
    ap_log_rerror(APLOG_MARK, APLOG_TRACE2, 0, r, "xsendfile: sending %d bytes", (int)finfo.size);
